...
import type { AlignEnum, BalloonProps, BalloonState } from './types';
...
class Balloon extends React.Component<BalloonProps, BalloonState> {
    ...
    _onClose: MouseEventHandler<HTMLAnchorElement> = function (e) {
    ...
    _onPosition(res: { align: string[] }) {
        ...
        for (const key in alignMap) {
            if (alignMap[key as AlignEnum][alignKey] === newAlign) {
                resAlign = key as AlignEnum;
    ...
    beforePosition = (
        result: { config: { placement: AlignEnum }; style: { left: number; top: number } },
    ...
    render() {
        ...
        const otherProps: {
            ...
            onPosition?: (res: { align: string[] }) => void;
        } = {
            delay: delay,
